...
r"""Script to download the Imagenet dataset and upload to s3.
...
flags.DEFINE_string(
    's3_output_path', None, 'S3 path for uploading the dataset.')
flags.DEFINE_string(
...
flags.DEFINE_boolean(
    's3_upload', True, 'Set to false to not upload to S3.')
...
def main(argv):  # pylint: disable=unused-argument
  ...
  if FLAGS.s3_upload and FLAGS.s3_output_path is None:
    raise ValueError('S3 output path must be provided.')
  elif FLAGS.s3_upload and not FLAGS.s3_output_path.startswith('s3://'):
    raise ValueError('S3 output path must start with s3://')
